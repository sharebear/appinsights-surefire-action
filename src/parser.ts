...
type TestCaseResult = 'passed' | 'failed' | 'error' | 'skipped'
...
const parseSurefireXML = async (
  input: convertableToString
): Promise<TestCaseDomain[]> => {
  const xmlAsJson = await parseStringPromise(input)
  const parsedJson = SurefireTestReport.parse(xmlAsJson)
  return parsedJson.testsuite.testcase.map(
    (tc: TestCase): TestCaseDomain => {
      return {
